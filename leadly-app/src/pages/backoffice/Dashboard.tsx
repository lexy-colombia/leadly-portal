    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <StatCard
          ...
          label="Líneas de WhatsApp activas"
          value={stats ? `${stats.activeLines} / ${stats.totalLines}` : '—'}
        />
        <StatCard icon={<ChatBubbleIcon width={18} height={18} />} label="Conversaciones de hoy" value={stats ? String(stats.conversationsToday) : '—'} />
      </div>

      <Card>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="font-semibold text-brand-800">Clientes recientes</h2>
          <Link to="/backoffice/clientes" className="text-xs font-medium text-accent-600 hover:text-accent-700">
            Ver todos
          </Link>
        </div>
        {!tenants && <PageSpinner />}
        {tenants && recentTenants.length === 0 && <EmptyState>Todavía no hay clientes registrados.</EmptyState>}
                ...
                >
                  <span className="flex min-w-0 items-center gap-3">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block truncate text-xs text-brand-400">Desde {formatDate(tenant.created_at)}</span>
                    </span>
                  </span>
