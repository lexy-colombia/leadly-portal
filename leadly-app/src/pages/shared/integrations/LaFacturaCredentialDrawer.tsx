            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <IntegrationFieldLabel
                  ...
                  badge={userConfigured && <Badge tone="success">{t('integrations.configured')}</Badge>}
                />
                <Input
                  id="lafactura-user"
                  value={apiUser}
              ...
              <div>
                <IntegrationFieldLabel
                  ...
                  badge={passwordConfigured && <Badge tone="success">{t('integrations.configured')}</Badge>}
                />
                <Input
          ...
          <IntegrationSection title={t('integrations.section.settings')}>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <Select id="lafactura-mode" value={mode} onChange={(e) => handleModeChange(e.target.value as 'sandbox' | 'production')}>
              ...
              <div>
                ...
                <Input id="lafactura-iva-rate" type="number" min={0} max={100} step={0.5} value={ivaRate} onChange={(e) => setIvaRate(e.target.value)} />
              </div>
            </div>

            <div>
              <IntegrationFieldLabel htmlFor="lafactura-base-url" label={t('integrations.lafactura.apiBaseUrl')} />
              <Input id="lafactura-base-url" value={apiBaseUrl} onChange={(e) => setApiBaseUrl(e.target.value)} placeholder="https://api.lafactura.co" />
            </div>

            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <IntegrationFieldLabel htmlFor="lafactura-id-api" label={t('integrations.lafactura.idApi')} />
                <Input id="lafactura-id-api" value={idApi} onChange={(e) => setIdApi(e.target.value)} />
              </div>
              <div>
                <IntegrationFieldLabel htmlFor="lafactura-range-key" label={t('integrations.lafactura.rangeKey')} />
                <Input id="lafactura-range-key" value={rangeKey} onChange={(e) => setRangeKey(e.target.value)} />
              </div>
            </div>
          </IntegrationSection>
