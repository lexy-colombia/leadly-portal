...
const DIAN_HABILITACION_URL =
  "https://vpfe-hab.dian.gov.co/WcfDianCustomerServices.svc";
const DIAN_PRODUCTION_URL =
...
export function DianDirectoCredentialDrawer({
  ...
  const [resolutionSyncError, setResolutionSyncError] = useState<string | null>(
    null,
  );
  const [resolutionSyncNote, setResolutionSyncNote] = useState<string | null>(
    null,
  );

  const [taxTypes, setTaxTypes] = useState<TaxType[]>([]);
  const [withholdingConfigs, setWithholdingConfigs] = useState<
    ...
    Promise.all([
      ...
      .then(async ([credential, profile, types, configs]) => {
        ...
        setCertificateFilename(
          typeof config.certificate_filename === "string"
            ? config.certificate_filename
            : "",
        );
        setConfiguredSecrets(
          credential
            ? await getIntegrationCredentialConfiguredSecrets(credential.id)
            : [],
        );
        ...
        setResolutionRangeFrom(
          profile?.resolution_range_from != null
            ? String(profile.resolution_range_from)
            : "",
        );
        setResolutionRangeTo(
          profile?.resolution_range_to != null
            ? String(profile.resolution_range_to)
            : "",
        );
      ...
      .catch((err) =>
        setError(
          err instanceof Error ? err.message : t("integrations.errors.load"),
        ),
      );
  }, [open, tenantId]);
  ...
  async function handleModeChange(next: "sandbox" | "production") {
    ...
    try {
      ...
      const pointsToOther =
        next === "production"
          ? webserviceUrl.includes("vpfe-hab.")
          : webserviceUrl.includes("//vpfe.");
      if (!webserviceUrl.trim() || pointsToOther) {
        setWebserviceUrl(
          next === "production" ? DIAN_PRODUCTION_URL : DIAN_HABILITACION_URL,
        );
      }
    } catch (err) {
      setError(
        err instanceof Error ? err.message : t("integrations.errors.save"),
      );
    }
  }

  async function handleCertificateSelect(
  ...
  function pickActiveRange(ranges: DianNumberingRange[]): DianNumberingRange {
    ...
    const withParsedValidUntil = ranges.filter(
      (r) => r.validUntil && !Number.isNaN(Date.parse(r.validUntil)),
    );
    const stillValid = withParsedValidUntil.filter(
      (r) => (r.validUntil as string).slice(0, 10) >= today,
    );
    const pool =
      stillValid.length > 0
        ? stillValid
        : withParsedValidUntil.length > 0
          ? withParsedValidUntil
          : ranges;
  ...
  async function handleSyncResolution() {
    ...
    try {
      ...
      if (result.ranges.length === 0) {
        setResolutionSyncError(
          result.operationDescription ??
            t("integrations.dianDirecto.syncNoRanges"),
        );
        return;
      }
      ...
      if (chosen.rangeFrom != null)
        setResolutionRangeFrom(String(chosen.rangeFrom));
      if (chosen.rangeTo != null) setResolutionRangeTo(String(chosen.rangeTo));
      if (chosen.validFrom)
        setResolutionValidFrom(chosen.validFrom.slice(0, 10));
      if (chosen.validUntil)
        setResolutionValidUntil(chosen.validUntil.slice(0, 10));
      ...
      setResolutionSyncNote(
        result.ranges.length > 1
          ? t("integrations.dianDirecto.syncMultipleFound", {
              count: String(result.ranges.length),
            })
          : t("integrations.dianDirecto.syncApplied"),
      );
    } catch (err) {
      setResolutionSyncError(
        err instanceof Error
          ? err.message
          : t("integrations.dianDirecto.syncError"),
      );
    } finally {
  ...
  async function handleSubmit() {
    ...
    try {
      if (certificatePassword.trim())
        await setIntegrationCredentialSecret(
          PROVIDER_KEY,
          tenantId,
      ...
      const isConfigured = !!(
        ...
        resolutionRangeFrom.trim() &&
        resolutionRangeTo.trim()
      );
      await updateTenantDianProfile(tenantId, {
        ...
        resolution_range_from: resolutionRangeFrom.trim()
          ? Number(resolutionRangeFrom)
          : null,
        resolution_range_to: resolutionRangeTo.trim()
          ? Number(resolutionRangeTo)
          : null,
      ...
      setError(message);
      toast.error(t("common.toast.saveFailed"), { description: message });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleAddWithholding() {
    if (
      !tenantId ||
      !newWithholdingType ||
      !newWithholdingConcept.trim() ||
      !newWithholdingRate.trim()
    )
      return;
    try {
  ...
  return (
    <Drawer
      open={open}
      onClose={onClose}
      title={t("integrations.dianDirecto.title")}
      description={description}
      size="lg"
    >
      {!loaded && !error && (
        <p className="text-xs text-brand-400">{t("common.status.loading")}</p>
      )}
        ...
        <div className="space-y-4">
          <div className="rounded-xl bg-brand-50/60 p-3">
            <p className="text-xs leading-snug text-brand-500">
              {t("integrations.dianDirecto.hint")}
            </p>
          </div>
          ...
          <IntegrationSection
            title={t("integrations.dianDirecto.section.certificate")}
          >
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {certificateFilename
                  ? t("integrations.dianDirecto.certificateReplace")
                  : t("integrations.dianDirecto.certificateUpload")}
              </Button>
              {uploadingCert && (
          ...
          >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-mode"
                  label={t("integrations.mode")}
                />
                <Select
                  value={mode}
                  onValueChange={(v) =>
                    handleModeChange(v as "sandbox" | "production")
                  }
                >
                  <SelectTrigger
                    id="dian-mode"
                ...
                {mode === "production" && (
                  <p className="mt-1 text-[10px] leading-tight text-amber-600">
                    {t("integrations.dianDirecto.environmentHint")}
                  </p>
                )}
              </div>
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-fiscal-regime"
                  label={t("integrations.dianDirecto.fiscalRegime")}
                />
                <Select
                  value={fiscalRegime}
                  onValueChange={(v) =>
                    setFiscalRegime(
                      v as "responsable_iva" | "no_responsable_iva",
                    )
                  }
                >
                  <SelectTrigger
                    id="dian-fiscal-regime"
                    className={`w-full ${FIELD_CLASS}`}
                  >
                    <SelectValue
            ...
            <div className="flex items-center justify-between rounded-lg border border-brand-100 px-3 py-2.5">
              <p className="text-xs font-medium text-brand-700">
                {t("integrations.dianDirecto.selfWithholding")}
              </p>
              <Switch
                checked={isSelfWithholdingAgent}
                onCheckedChange={setIsSelfWithholdingAgent}
              />
            </div>

            <div>
              <IntegrationFieldLabel
                htmlFor="dian-software-id"
                label={t("integrations.dianDirecto.softwareId")}
              />
              <Input
                id="dian-software-id"
                value={softwareId}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {mode === "sandbox" && (
                <div>
                  <IntegrationFieldLabel
                    htmlFor="dian-test-set-id"
                    label={t("integrations.dianDirecto.testSetId")}
                  />
                  <Input
                    id="dian-test-set-id"
                    value={testSetId}
              ...
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-webservice-url"
                  label={t("integrations.dianDirecto.webserviceUrl")}
                />
                <Input
                  id="dian-webservice-url"
                  value={webserviceUrl}
                  onChange={(e) => setWebserviceUrl(e.target.value)}
                  placeholder={DIAN_HABILITACION_URL}
                  className={FIELD_CLASS}
                />
              </div>
            </div>
          ...
          <IntegrationSection
            ...
            action={
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={handleSyncResolution}
                disabled={syncingResolution}
              >
                <RefreshCwIcon className="size-3.5" />
                {syncingResolution
                  ? t("integrations.dianDirecto.syncing")
                  : t("integrations.dianDirecto.sync")}
              </Button>
            }
          >
            {resolutionSyncError && (
              <FieldError message={resolutionSyncError} />
            )}
            {resolutionSyncNote && (
              <p className="text-xs text-emerald-600">{resolutionSyncNote}</p>
            )}
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-resolution-number"
                  label={t("integrations.dianDirecto.resolutionNumber")}
                />
                <Input
                  id="dian-resolution-number"
              ...
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-resolution-prefix"
                  label={t("integrations.dianDirecto.resolutionPrefix")}
                />
                <Input
                  id="dian-resolution-prefix"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-resolution-from"
                  label={t("integrations.dianDirecto.resolutionRangeFrom")}
                />
                <Input
                  id="dian-resolution-from"
                  type="number"
                  value={resolutionRangeFrom}
                  onChange={(e) => setResolutionRangeFrom(e.target.value)}
                  className={FIELD_CLASS}
                />
              </div>
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-resolution-to"
                  label={t("integrations.dianDirecto.resolutionRangeTo")}
                />
                <Input
                  id="dian-resolution-to"
                  type="number"
                  value={resolutionRangeTo}
                  onChange={(e) => setResolutionRangeTo(e.target.value)}
                  className={FIELD_CLASS}
                />
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-resolution-valid-from"
                  label={t("integrations.dianDirecto.resolutionValidFrom")}
                />
                <Input
                  id="dian-resolution-valid-from"
                  type="date"
                  value={resolutionValidFrom}
                  onChange={(e) => setResolutionValidFrom(e.target.value)}
                  className={FIELD_CLASS}
                />
              </div>
              <div>
                <IntegrationFieldLabel
                  htmlFor="dian-resolution-valid-until"
                  label={t("integrations.dianDirecto.resolutionValidUntil")}
                />
                <Input
                  id="dian-resolution-valid-until"
                  type="date"
                  value={resolutionValidUntil}
                  onChange={(e) => setResolutionValidUntil(e.target.value)}
                  className={FIELD_CLASS}
                />
              </div>
            </div>
            ...
            <div>
              <IntegrationFieldLabel
                htmlFor="dian-credit-note-prefix"
                label={t("integrations.dianDirecto.creditNotePrefix")}
              />
              <Input
                ...
                placeholder={t(
                  "integrations.dianDirecto.creditNotePrefixPlaceholder",
                )}
                className={`sm:w-1/2 ${FIELD_CLASS}`}
              />
            </div>
          ...
          <IntegrationSection
            title={t("integrations.dianDirecto.section.withholding")}
          >
            {withholdingConfigs.length > 0 && (
              <div className="space-y-1.5">
                {withholdingConfigs.map((c) => (
                  <div
                    key={c.id}
                    className="flex items-center justify-between gap-2 rounded-lg border border-brand-100 px-2.5 py-1.5 text-xs"
                  >
                    <span className="min-w-0 flex-1 truncate">
                      <span className="font-medium text-brand-700">
                        {taxTypes.find((t2) => t2.code === c.tax_type_code)
                          ?.name ?? c.tax_type_code}
                      </span>
                    ...
                    <button
                      type="button"
                      onClick={() => handleRemoveWithholding(c.id)}
            ...
            <div className="grid grid-cols-[1fr_1fr_80px_auto] items-end gap-2">
              <div>
                <Select
                  value={newWithholdingType}
                  onValueChange={setNewWithholdingType}
                >
                  <SelectTrigger className={`w-full ${FIELD_CLASS}`}>
                    <SelectValue
                      placeholder={t(
                        "integrations.dianDirecto.withholdingType",
                      )}
                    />
                  </SelectTrigger>
                  <SelectContent>
