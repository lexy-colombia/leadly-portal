...
import { BoxIcon, ChevronLeftIcon, MailIcon, PencilIcon, PhoneIcon, PlusIcon, UserIcon } from '@/components/atoms/icons'
...
/** Label-above-value field, same shape ClientDetail uses for its data
...
export function ProductDetail() {
  ...
  // types/domain.ts) -- el total disponible/reservado ahora se deriva de
  // la suma por bodega (product_stock), no de un contador plano en el
  // producto. null mientras stockByWarehouse todavía no cargó -- lowStock
  // se resuelve en cuanto llega, no bloquea el resto de la página.
          ...
          {product.track_inventory && (
            <Section title={t('inventory.product.section.stockByWarehouse')}>
              <dl className="mb-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                <Field label={t('products.detail.fields.available')} value={<span className={lowStock ? 'font-semibold text-red-600' : ''}>{totalAvailable ?? '-'}</span>} />
                <Field label={t('products.detail.fields.reserved')} value={totalReserved ?? '-'} />
