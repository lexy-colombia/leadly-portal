...
export function Clients() {
  ...
  const [lastContact, setLastContact] = useState<Map<string, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    listProfilesByTenant(tenantId)
      .then(setAgents)
      .catch(() => {});
    listLastContactTimesByTenant(tenantId)
      .then(setLastContact)
      .catch(() => {});
  }, [profile?.tenant_id]);
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("contacts.errors.delete"),
      );
    } finally {
      setDeleting(false);
    }
  ...
  return (
    <div className="animate-fade-in space-y-3">
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-2xl border border-brand-100 bg-brand-50/40 p-3">
        ...
        {allTags.length > 0 && (
          <FilterField label={t("contacts.filters.labels.tag")}>
            <ComboboxFilter
        ...
        <span className="shrink-0 pb-1.5 text-xs text-brand-400">
          ...
          {t(
            (filtered?.length ?? 0) === 1
              ? "contacts.count.singular"
              : "contacts.count.plural",
          )}
        </span>
        ...
        <Card>
          <EmptyState>
            {contacts && contacts.length > 0
              ? t("contacts.emptyState.noMatch")
              : t("contacts.emptyState.none")}
          </EmptyState>
        </Card>
      )}
