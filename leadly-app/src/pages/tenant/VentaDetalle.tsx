...
import { getOrder, listOrderItems, updateOrderStatus, updateOrderFields, updateOrderItemsAndTotals } from '../../lib/api/orders'
...
import { listPaymentsForOrder, deletePayment } from '../../lib/api/orderPayments'
...
import type { CrmContactAddress, CrmOrderPayment, CrmAttachment, OrderStatus, OrderPaymentMethod } from '../../types/domain'
...
import { AlertIcon, ClockIcon, PlusIcon, TrashIcon } from '../../components/icons'
...
/** Same small pill used across the CRM (ContactoDetalle.tsx notes/PQR) to
...
export function VentaDetalle() {
  ...
  const [payments, setPayments] = useState<CrmOrderPayment[] | null>(null)
  const [comments, setComments] = useState<OrderCommentWithAuthor[] | null>(null)
  const [attachmentsByComment, setAttachmentsByComment] = useState<Record<string, CrmAttachment[]>>({})
        ...
        .then((attachments) => {
          setAttachmentsByComment((prev) => {
            ...
            for (const a of attachments) {
              if (!a.order_comment_id) continue
              next[a.order_comment_id] = [...(next[a.order_comment_id] ?? []), a]
            }
            return next
          })
        })
  ...
  async function handleVoid() {
    ...
    try {
      ...
      // keeps its full history). The stock-effect trigger on crm_orders
      // (20260809000001_order_stock_effects.sql) still reacts to this
      // transition the same way regardless of the previous status.
  ...
  if (order === null) {
    return (
      <div className="space-y-4">
        <p className="text-brand-500">No encontramos esta venta.</p>
  ...
  return (
    <div className="space-y-3">
      {/* Sin breadcrumb propio -- AppShell ya renderiza "← Ventas" en el
      ...
      <Card padded={false} className="overflow-hidden">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 p-3 sm:p-4">
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-brand-800">Venta {order.number}</span>
            <Badge tone={STATUS_TONE[order.status]}>{STATUS_LABEL[order.status]}</Badge>
          </div>
          <div className="flex items-center gap-3 text-xs text-brand-500">
            <span className="flex items-center gap-1">
              <ClockIcon width={12} height={12} /> {formatDateTime(order.created_at)}
            </span>
        ...
        <div className="grid grid-cols-1 gap-4 border-t border-brand-100 p-3 sm:p-4 lg:grid-cols-[1fr_1px_1fr_1px_1fr]">
          <div className="space-y-2.5">
            ...
            <InfoField label="Oportunidad">{order.opportunity?.title ?? '—'}</InfoField>
          </div>
          ...
          <div className="space-y-2.5">
            <InfoField
              label="Envío"
              action={
                <button type="button" onClick={() => setAddressDrawerOpen(true)} className="text-brand-400 hover:text-accent-600" aria-label="Nueva dirección">
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <p className="text-xs font-bold text-brand-800">Pagos</p>
              <button
                type="button"
            ...
            {balance > 0 ? (
              <div className="flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-medium text-red-700">
                <AlertIcon width={13} height={13} /> Pendiente: {formatCurrency(balance, order.currency)}
              </div>
            ) : (
              payments &&
              payments.length > 0 && (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-xs font-medium text-emerald-700">Pagado en su totalidad</div>
              )
            )}
                ...
                {payments.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-2 text-xs text-brand-600">
                    <span className="min-w-0 truncate">
                      {METHOD_LABEL[p.method]} · {formatDate(p.paid_at)}
                    </span>
                    <span className="flex shrink-0 items-center gap-1.5">
                      <span className="font-medium text-brand-800">{formatCurrency(p.amount, p.currency)}</span>
                      <button type="button" onClick={() => setDeletePaymentId(p.id)} className="text-brand-300 hover:text-red-600" aria-label="Eliminar pago">
            ...
            {payments && payments.length === 0 && balance === 0 && <p className="text-xs text-brand-400">Sin pagos registrados.</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 border-t border-brand-100 p-3 sm:p-4 lg:grid-cols-2">
          <div>
            <p className="mb-1.5 text-xs font-bold text-brand-800">Comentarios</p>
            ...
            {comments && comments.length === 0 && <p className="text-xs text-brand-400">Sin comentarios todavía.</p>}
            {comments && comments.length > 0 && (
              <ul className="divide-y divide-brand-100">
                {commentsPageData.items.map((c) => (
                  <li key={c.id} className="flex items-start gap-2 py-2 first:pt-0 last:pb-0">
                    <InitialsAvatar name={c.created_by_ai ? 'IA' : (c.author?.full_name ?? 'Agente')} size="xs" />
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 flex items-center gap-1.5 text-xs text-brand-400">
                        {c.created_by_ai ? 'Asistente de IA' : (c.author?.full_name ?? 'Agente')} · {formatDateTime(c.created_at)}
                        {c.created_by_ai && <AiBadge />}
                      </p>
                    </div>
                  </li>
                ))}
          ...
          <div>
            <p className="mb-1.5 text-xs font-bold text-brand-800">Notas</p>
            <Textarea value={notesDraft} onChange={(e) => setNotesDraft(e.target.value)} rows={2} placeholder="Agrega una nota sobre esta venta..." className="text-sm" />
            {notesDirty && (
            ...
            {relatedTasks && relatedTasks.length > 0 && (
              <div className="mt-3 border-t border-brand-100 pt-3">
                <p className="mb-1.5 text-xs font-bold text-brand-800">Tareas relacionadas</p>
                <ul className="space-y-1">
                  {relatedTasks.map((t) => (
                    <li key={t.id} className="flex items-center justify-between gap-2 text-sm">
                      <span className="min-w-0 truncate text-brand-600">{t.title}</span>
                      <Badge tone={t.status === 'completada' ? 'success' : 'warning'}>{t.status === 'completada' ? 'Listo' : 'Pendiente'}</Badge>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {/* Fila 4: Productos (editable) + resumen de la orden abajo a la derecha */}
        <div className="border-t border-brand-100 p-3 sm:p-4">
          <div className="mb-1.5 flex items-center justify-between">
            <p className="text-xs font-bold text-brand-800">Productos</p>
            {itemsDirty && (
          ...
          {items && items.length === 0 && <EmptyState>Sin productos en esta venta.</EmptyState>}
          {items && items.length > 0 && (
            <p className="mt-1.5 text-xs text-brand-400">
              {items.length} producto{items.length === 1 ? '' : 's'} · Cantidad total: {totalQuantity}
            </p>
          )}

          <div className="mt-3 flex justify-end">
            <div className="w-full rounded-xl border border-brand-100 p-3 sm:w-72">
              <p className="mb-0.5 text-xs font-bold text-brand-800">Resumen de la orden</p>
      ...
      <ConfirmDialog
        ...
        title="Eliminar pago"
        description="¿Eliminar este pago? Se puede volver a registrar si fue un error."
      />
      ...
      <ConfirmDialog
        ...
        loading={voiding}
        title="Anular pedido"
        description="El pedido queda marcado como anulado (no se elimina, conserva su historial). Si tenía stock reservado o descontado, se restaura."
