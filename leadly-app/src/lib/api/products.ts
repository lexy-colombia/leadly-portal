...
export type ProductWithImages = CrmProduct & {
...
export async function listProducts(tenantId: string): Promise<ProductWithImages[]> {
  const { data, error } = await supabase
    .from('crm_products')
    .select('*, images:crm_product_images(*), category:crm_product_categories(id, name, color), supplier:crm_suppliers(id, name)')
...
export type ProductDetail = CrmProduct & {
  images: CrmProductImage[]
 ...
 * details) -- ProductoDetalle.tsx shows more than the list/edit form does,
...
export async function getProduct(id: string): Promise<ProductDetail | null> {
  const { data, error } = await supabase
    .from('crm_products')
    .select(
      '*, images:crm_product_images(*), category:crm_product_categories(id, name, color, description), supplier:crm_suppliers(id, name, contact_name, phone, email)',
    )
    .eq('id', id)
    .is('deleted_at', null)
    .maybeSingle()
...
export async function createProduct(input: ProductInput): Promise<CrmProduct> {
  const { data, error } = await supabase.from('crm_products').insert(input).select().single()
  if (error) throw error
  return data
}

export async function updateProduct(id: string, input: Partial<ProductInput>): Promise<CrmProduct> {
  const { data, error } = await supabase.from('crm_products').update(input).eq('id', id).select().single()
  if (error) throw error
  return data
}
...
export async function deleteProduct(id: string): Promise<void> {
  ...
  const { error } = await supabase.from('crm_products').update({ deleted_at: new Date().toISOString(), deleted_by: user?.id ?? null }).eq('id', id)
  if (error) throw error
}
 ...
 * inserts the crm_product_images row that points at it -- the storage
 * upload and the DB row are two separate steps, unlike the tenant logo's
 * single upsert-by-fixed-path, because a product can have several images. */
export async function uploadProductImage(tenantId: string, productId: string, file: File, displayOrder: number): Promise<CrmProductImage> {
...
export async function deleteProductImage(id: string, storagePath: string): Promise<void> {
  const { error } = await supabase.from('crm_product_images').delete().eq('id', id)
  if (error) throw error
