...
export function SignedImage({ storagePath, alt, className }: { storagePath: string; alt?: string; className?: string }) {
  ...
  if (failed) {
    return <div className={`flex items-center justify-center rounded-lg bg-brand-50 text-xs text-brand-400 ${className ?? ''}`}>No disponible</div>
  }
  if (!url) {
  ...
  return (
    <a href={url} target="_blank" rel="noreferrer" className="block">
      <img src={url} alt={alt ?? 'Imagen adjunta'} className={`rounded-lg object-cover ${className ?? ''}`} />
    </a>
  )
}
