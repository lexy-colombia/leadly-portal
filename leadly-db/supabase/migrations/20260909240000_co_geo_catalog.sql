-- Catálogo real de departamentos y municipios de Colombia (DIVIPOLA) --
-- pedido explícito del usuario 2026-09-09: en vez de que cada tenant/
-- cliente tenga que escribir a mano su código DANE (como quedó en la
-- migración anterior 20260909230000), se carga el catálogo oficial
-- completo una sola vez y de ahí en más se ELIGE de una lista (nombre real,
-- nunca un código a ciegas) -- la lista misma ya trae el código correcto.
--
-- Se revisó primero si el proyecto Lexy (gyartmmdixflptwpanrp, el otro
-- producto de la misma organización) ya tenía algo así para reusar --
-- tiene el mismo problema sin resolver (`company_fiscal_profiles.
-- divipola_city_code` es un campo de texto suelto, sin catálogo detrás),
-- así que no había nada que traer.
--
-- Datos tomados de la API oficial de datos abiertos del gobierno
-- colombiano (datos.gov.co, dataset "DIVIPOLA - Códigos municipios",
-- resource gdxc-w37w) el 2026-09-09 -- NO transcritos a mano, generados
-- programáticamente desde la respuesta JSON de esa API para no arriesgar
-- un error de tipeo en un código que la DIAN valida como RECHAZO real
-- (reglas FAJ09/FAJ12/FAJ29/FAJ32, ver buildInvoiceXml.ts). 33
-- departamentos + 1122 municipios -- coincide exactamente con el conteo
-- oficial de DIVIPOLA.
--
-- Los nombres se guardan en formato legible (Title Case, ej. "Bogotá,
-- D.C." en vez de "BOGOTÁ, D.C." como los devuelve la API) -- el nombre no
-- es lo que la DIAN valida estrictamente (FAJ10/FAJ30 son solo
-- Notificación, no Rechazo), lo que importa de verdad es el código.
...
create index if not exists cities_department_code_idx on public.cities(department_code);
revoke all on public.cities from anon;
grant select on public.cities to anon, authenticated;

-- Catálogo de solo lectura para todo el mundo autenticado (no tiene
-- tenant_id -- es información pública, igual que tax_types/
-- dian_document_types) -- sin RLS habilitado a propósito, mismo criterio
-- que esas dos tablas.

-- Referencia real en vez de texto suelto -- las columnas ya existían
-- (migración 20260909230000) como `text` libre; ahora apuntan al
-- catálogo. Nullable en los dos casos: mientras el tenant/cliente no elija
-- una ciudad, el campo queda vacío (no se inventa un valor por default).
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'tenant_dian_profile_city_code_fkey') then
    alter table public.tenant_dian_profile add constraint tenant_dian_profile_city_code_fkey foreign key (city_code) references public.cities(code);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'tenant_dian_profile_state_code_fkey') then
    alter table public.tenant_dian_profile add constraint tenant_dian_profile_state_code_fkey foreign key (state_code) references public.departments(code);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'contact_addresses_city_code_fkey') then
    alter table public.contact_addresses add constraint contact_addresses_city_code_fkey foreign key (city_code) references public.cities(code);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'contact_addresses_state_code_fkey') then
    alter table public.contact_addresses add constraint contact_addresses_state_code_fkey foreign key (state_code) references public.departments(code);
  end if;
end $$;

-- Auto-generado desde la API oficial de datos.gov.co (dataset DIVIPOLA
-- gdxc-w37w, 'DIAN- Codigos municipios', consultado 2026-09-09) -- NO
-- transcrito a mano, para no arriesgar un error de tipeo en un código
-- que la DIAN valida como rechazo real (FAJ09/FAJ12/FAJ29/FAJ32). 33
-- departamentos + 1122 municipios, igual al conteo oficial de DIVIPOLA.
...
insert into public.cities (code, name, department_code) values
  ('05001', 'Medellín', '05'),
  ('05002', 'Abejorral', '05'),
  ('05004', 'Abriaquí', '05'),
  ('05021', 'Alejandría', '05'),
