-- Real test feedback: quoting 500 pares of botas (2 disponibles) blocked
-- create_quote entirely with "Stock insuficiente", which also meant
-- resolveOrCreateOpportunityForQuote never ran -- the lead vanished with no
-- cotización AND no oportunidad. A cotización is a price estimate, not a
-- stock commitment; rejecting it at that point was wrong. Stock now only
-- gets checked at confirm_quote (whatsapp-ai-tools/index.ts), the actual
-- moment a reservation turns into a real inventory deduction -- create_quote
-- itself never fails on stock anymore (see its own updated code comment).
-- Also fixed in the same deploy: create_opportunity's dedup was reusing a
-- STALE open opportunity from an unrelated earlier product interest instead
-- of updating it, so the pipeline kept showing "Interesado en camisetas"
-- even after the same contact asked about botas -- it now updates the
-- existing one's title/value/description instead of just returning it as-is.
...
- create_quote: cuando el cliente confirme qué productos y cantidades quiere. Si no sabés el nombre exacto de un producto o su precio, consultá list_catalog_products primero (habilidad de Catálogo) -- nunca inventes un producto, precio o cantidad. Una cotización es una propuesta de precio, no un compromiso de stock -- creala aunque la cantidad pedida sea mayor a lo disponible, nunca la rechaces vos ni le digas al cliente que no hay suficiente en esta etapa. La disponibilidad real se revisa recién al confirmar (ver confirm_quote).
...
- confirm_quote: en cuanto el cliente confirme explícitamente que quiere seguir adelante con la compra de su cotización más reciente -- no lo dejes pendiente si ya dijo que sí. Acá sí se revisa el stock real: si no alcanza, la herramienta te va a decir el máximo disponible -- contale eso al cliente con claridad (ej. "de ese producto solo puedo confirmar 2 unidades, ¿querés que ajustemos la cantidad?") en vez de insistir en confirmar la cantidad completa.
